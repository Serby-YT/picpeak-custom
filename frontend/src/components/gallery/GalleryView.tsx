...
export const GalleryView: React.FC<GalleryViewProps> = ({ slug, event }) => {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'name' | 'size' | 'rating' | 'capture_date'>('capture_date');
    ...
    photos.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'capture_date':
          ...
          // Ascending — chronological order, oldest/first-taken photo first.
          const captureDateA = a.captured_at || a.uploaded_at;
          const captureDateB = b.captured_at || b.uploaded_at;
          return new Date(captureDateA).getTime() - new Date(captureDateB).getTime();
        case 'date':
        default:
